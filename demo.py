...
from os import getenv # This is used to connect to the .env file with sensitive information
import psycopg2 # This is used to connect to the database

# Step 2: We will load the .env file
# Since the .env file is in the same directory, I just need to use the load_dotenv() method below
load_dotenv()

# Steo 3: Create a class
class PGSQL:
    # Step 3a: Create a user, password, server variable by accessing the .env file
    # We will keep these items as a secret under our class by using the __ method (thunder is the double underscore)
    # You keep it a secret so that no one is able to gain access to the infomation or instances classified with the thunder method
    __user = getenv("DB") #These are going to be pulled from the .env folder with the getenv() method
    __password = getenv("PASSWORD")
    __server = getenv("SERVER")

    # Step 3b: Connect and access the database:
    # You want to have this under the thunder method so you will keep the connection to the database out of the hands of the user
    __pg_con = psycopg2.connect( #pyscopg2 is the driver for Python to connect to PostgreSQL
        dbname = __user, # we give it the dbname( database name) which is the thunder user from class PGSQL
        user = __user,
        password = __password,
        host = __server
    )
...
if __name__ =='__main__':
   c = PGSQL()
   c.create_tables('/Users/investmentguy/Documents/coding_temple/bonfire_124_sql_day_1/bonfire_124_sql_day_1/amazon_mock_create.sql')
   c.insert_data("/Users/investmentguy/Documents/coding_temple/bonfire_124_sql_day_1/bonfire_124_sql_day_1/amazon_mock_insert.sql")